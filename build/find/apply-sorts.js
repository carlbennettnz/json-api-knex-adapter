...
function validateSorts(model, sorts) {
    ...
    if (invalidSorts.length > 0) {
        throw invalidSorts.map(({ field }) => new json_api_1.Error(400, undefined, 'Invalid sort', `The attribute '${field}' does not exist as an attribute or relationship on this model.'`));
    }
}
